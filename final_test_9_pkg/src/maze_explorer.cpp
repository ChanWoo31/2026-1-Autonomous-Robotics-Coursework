...
public:
    MazeExplorer() : Node("maze_explorer")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "maze_explorer v13 final_test_9 dead-end memory loaded");
    }
...
private:
    ...
    struct DeadEndZone
    {
        double x;
        double y;
        double start_path_distance;
        double projection;
    };

    std::vector<BlacklistEntry> blacklist_;
    std::vector<DeadEndZone> dead_end_zones_;

    const double BLACKLIST_RADIUS = 0.22;
    const double BLACKLIST_TTL_SUCCESS = 6.0;
    const double BLACKLIST_TTL_FAILURE = 30.0;
    const double BLACKLIST_TTL_CANCEL = 25.0;
    const double DEAD_END_HARD_RADIUS = 0.32;
    const double DEAD_END_BRANCH_RADIUS = 0.90;
    const double DEAD_END_PATH_ALLOWANCE = 0.55;
    const double DEAD_END_PROJECTION_ALLOWANCE = 0.35;

    const double GOAL_CLEARANCE = 0.100;
    const double PATH_CLEARANCE = 0.080;
    const double PATH_BOTTLENECK_CLEARANCE = 0.090;
    ...
    const double MIN_FINAL_GOAL_CLEARANCE = 0.100;
    ...
    const double KEEP_MOVING_MIN_CLEARANCE = 0.100;
    const double KEEP_MOVING_RELAXED_CLEARANCE = 0.085;
    const double KEEP_MOVING_MIN_BOTTLENECK = 0.090;
    const double KEEP_MOVING_RELAXED_BOTTLENECK = 0.075;
    ...
    const double OPEN_RAY_GOAL_CLEARANCE = 0.100;
    const double LOOKAHEAD_PIVOT_CLEARANCE = 0.075;
    const double LOOKAHEAD_GOAL_CLEARANCE = 0.100;
    ...
    const double BRANCH_ESCAPE_MIN_BOTTLENECK = 0.075;
    ...
    bool getStartPathDistanceForPoint(
        ...
        double& start_path_distance)
    {
        ...
        start_path_distance = (*start_distances)[index] * map->info.resolution;
        return true;
    }

    void addDeadEndZone(double wx, double wy, const std::string& reason)
    {
        if (!latest_map_) {
            return;
        }

        double start_path_distance = 0.0;

        if (!getStartPathDistanceForPoint(
                latest_map_,
                wx,
                wy,
                start_path_distance)) {
            start_path_distance = current_start_path_distance_;
        }

        const double projection = forwardProjection(wx, wy);

        for (auto& zone : dead_end_zones_) {
            if (std::hypot(wx - zone.x, wy - zone.y) < DEAD_END_HARD_RADIUS) {
                zone.x = wx;
                zone.y = wy;
                zone.start_path_distance = std::max(
                    zone.start_path_distance,
                    start_path_distance
                );
                zone.projection = std::max(zone.projection, projection);
                return;
            }
        }

        dead_end_zones_.push_back({
            wx,
            wy,
            start_path_distance,
            projection
        ...
        if (dead_end_zones_.size() > 40) {
            dead_end_zones_.erase(dead_end_zones_.begin(), dead_end_zones_.begin() + 8);
        }

        RCLCPP_WARN(
            this->get_logger(),
            "dead-end zone 등록: x=%.2f, y=%.2f, start_path=%.2f, projection=%.2f, reason=%s",
            wx,
            wy,
            start_path_distance,
            projection,
        ...
        double start_path_distance) const
    {
        ...
        for (const auto& zone : dead_end_zones_) {
            const double spatial_distance = std::hypot(wx - zone.x, wy - zone.y);

            if (spatial_distance < DEAD_END_HARD_RADIUS) {
                return true;
            }

            if (spatial_distance < DEAD_END_BRANCH_RADIUS &&
                start_path_distance + DEAD_END_PATH_ALLOWANCE >=
                    zone.start_path_distance &&
                projection + DEAD_END_PROJECTION_ALLOWANCE >= zone.projection) {
                return true;
            }
        }
        ...
        double wy)
    {
        ...
        return isDeadEndCandidate(wx, wy, start_path_distance);
    }
    ...
    void cancelCurrentGoalAndBlacklist()
    {
        ...
        addDeadEndZone(current_goal_x_, current_goal_y_, "goal canceled or backtracking");
        ...
        auto selectGoal = [&](bool relaxed, int& best_mx, int& best_my,
            ...
            for (int my = 1; my < height - 1; ++my) {
                for (int mx = 1; mx < width - 1; ++mx) {
                    ...
                    if (isBlacklistedPoint(wx, wy, BLACKLIST_RADIUS) ||
                        isDeadEndCandidate(wx, wy, start_path_distance)) {
                        continue;
                    }
        ...
        geometry_msgs::msg::PoseStamped& goal)
    {
        ...
        for (int my = 1; my < height - 1; ++my) {
            for (int mx = 1; mx < width - 1; ++mx) {
                ...
                if (isDeadEndCandidate(wx, wy, start_path_distance)) {
                    continue;
                }
            ...
            for (int mx = 1; mx < width - 1; ++mx) {
                ...
                for (size_t cluster_i = 0; cluster_i < cluster.size(); ++cluster_i) {
                    ...
                    if (isBlacklistedPoint(goal_wx, goal_wy, BLACKLIST_RADIUS) ||
                        isDeadEndCandidate(goal_wx, goal_wy, start_path_distance)) {
                        continue;
                    }
        ...
        send_goal_options.goal_response_callback =
            [this](const GoalHandleNav2::SharedPtr& goal_handle) {
                if (!goal_handle) {
                    ...
                    addDeadEndZone(current_goal_x_, current_goal_y_, "Nav2 rejected goal");
                    goal_in_progress_ = false;
    ...
    void resultCallback(const GoalHandleNav2::WrappedResult& result)
    {
        ...
        if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
            if (moved_distance < 0.08 &&
                ...
                consecutive_goal_failures_++;
                addBlacklistPoint(current_goal_x_, current_goal_y_, BLACKLIST_TTL_FAILURE);
                addDeadEndZone(current_goal_x_, current_goal_y_, "success without real progress");
            ...
            RCLCPP_WARN(
                this->get_logger(),
                "Nav2 목표 실패. 해당 dead-end zone을 금지하고 재탐색합니다: x=%.2f, y=%.2f",
                current_goal_x_,
                current_goal_y_
            );

            consecutive_goal_failures_++;
            // 실패한 goal은 모든 생성기에서 제외해 같은 dead-end 가지를 반복하지 않는다.
            addBlacklistPoint(current_goal_x_, current_goal_y_, BLACKLIST_TTL_FAILURE);
            addDeadEndZone(current_goal_x_, current_goal_y_, "Nav2 goal failed");
            should_escape_after_result = canUseBackupRecovery();
            should_contact_reverse_after_result =
                nearestContactDistance() < CONTACT_OSCILLATION_DISTANCE;
            prefer_local_motion_after_result = true;
